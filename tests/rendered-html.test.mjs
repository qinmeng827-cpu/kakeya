...
test("keeps interactive models explicit about scope and removes fake estimates", async () => {
  ...
  assert.doesNotMatch(page, /HONG WANG · 王虹/);
  assert.match(page, /requestFullscreen/);
  assert.match(page, /cameraDistance/);
  assert.match(page, /drawGuide/);
  assert.match(page, /Math\.min\(width, height\) \* 0\.4/);
  ...
  assert.match(css, /\.proof-nav[\s\S]*?min-height:\s*680px/);
  assert.match(css, /\.proof-nav button[\s\S]*?font-size:\s*calc\(20px/);
  ...
  assert.match(css, /\.section-index[\s\S]*?font:\s*650 calc\(18px \+ var\(--type-ui-adjust\)\)/);
  ...
  assert.match(css, /\.profile-portrait\s*\{[\s\S]*?aspect-ratio:\s*16\s*\/\s*7/);
  assert.match(css, /\.person-profile-zahl\s*\{[\s\S]*?border-left:/);
  assert.match(css, /\.profile-portrait \.profile-image\s*\{[\s\S]*?object-fit:\s*cover/);
