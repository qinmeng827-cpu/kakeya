/*
 * Kakeya geometry and motion core, adapted from Terence Tao's tao-web.
 * Copyright © 2026 Terence Tao. Licensed under the MIT License.
 * Original: https://github.com/teorth/tao-web/tree/main/static/apps
...
//
// Two motion engines:
//   * convex-chord (Pal 1921): turn the needle inside a convex region 鈥?the disk and
//     the equilateral triangle (Fujiwara, the minimal convex Kakeya set).
//   * tangent-envelope: the needle is a chord of the tangent line to a curve 鈥?the
...
//          Cunningham鈥揝choenberg, Canad. J. Math. 17 (1965).
// ---------------------------------------------------------------------------
...
// --- 3. Deltoid / 3-cusped hypocycloid: the needle is the tangent chord (Cunningham鈥揝choenberg) ---
...
// --- 4. Bloom鈥揝choenberg star 饾攧_n (Cunningham鈥揝choenberg, Part II): tangent to n arcs ---
// n odd; points A_i in "star" order (step pi(n-1)/n) on the unit circle; arc 螕_i from A_i to
// A_{i+1} is tangent to the radii OA_i, OA_{i+1} at those points. The needle is the tangent to 螕_i;
// its direction turns pi/n per arc (pi over one pass). Geometry verified here; the exact tangent
// chord clipped to 饾攧_n (and the contraction to a true unit needle) is finalised with the visuals.
function starBS(n) {
  ...
  // the needle is the tangent line at P clipped to the two neighbouring arcs (Fig. 4: T0 on 螕_{i-1}, T1 on 螕_{i+1})
...
// --- Cunningham's modified star polygon (Cunningham鈥揝choenberg 1965, Part I 搂2) ---
...
// --- Cunningham's modified hypocycloid (Cunningham鈥揝choenberg 1965, Part I 搂1) ---
...
// Minimising over lambda gives area (2pi-2)/(pi+8) 鈮?0.1224*pi.
...
// constant-width case 鈥?every corner-to-opposite-arc chord has length exactly 1.
function reuleaux() {
...
// --- Besicovitch鈥揚erron tree + P谩l joins (Besicovitch 1963): area -> 0 (multiply connected) ---
// A demonstration: 2^p elementary triangles of height 1, translated (alpha-overlap) so they overlap. The
// needle pivots a sliver in each (that sweep is the shrinking Kakeya set); between triangles it makes a P谩l
// join 鈥?a detour that adds negligible area, shown here schematically. Deeper trees => smaller area.
function perronTree(p) {
  ...
  for (var k = 0; k < N; k++) {
    ...
    if (k < N - 1) (function (k) {   // P谩l join: detour to the next triangle (out along the line, across, back) 鈥?negligible area
